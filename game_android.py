...
# Grass surface (generated once)
grass_surface = None

def generate_grass_surface(width, height):
    """Generate a surface with many grass blades within an ellipse."""
    surf = pygame.Surface((width, height), pygame.SRCALPHA)
    cx, cy = width // 2, height // 2
    a, b = width // 2, height // 2
    # Grass blade colors: variations of green
    base_green = (80, 180, 60)
    dark_green = (50, 140, 40)
    light_green = (100, 200, 80)
    colors = [base_green, dark_green, light_green, (60, 160, 50), (90, 190, 70)]
    ...
    for _ in range(600):
        ...
        for attempt in range(100):
            ...
            # Could not find point after 100 attempts, skip this blade
            continue
        # Blade height and lean
        blade_height = random.randint(5, 25)
        lean = random.uniform(-3, 3)
        # Color variation
        color = random.choice(colors)
        # Slight brightness variation
        variation = random.randint(-20, 20)
        color = (max(0, min(255, color[0] + variation)),
                 max(0, min(255, color[1] + variation)),
                 max(0, min(255, color[2] + variation)))
        # Draw blade as a line
        end_x = x + lean
        end_y = y - blade_height
        width = random.randint(1, 2)
        pygame.draw.line(surf, color, (x, y), (end_x, end_y), width)
        # Optional: draw a tiny triangle at tip for thickness
        # pygame.draw.circle(surf, color, (int(end_x), int(end_y)), 1)
    # Add some small flowers (optional)
    for _ in range(30):
        for attempt in range(100):
            x = random.randint(0, width - 1)
            y = random.randint(0, height - 1)
            ...
            continue
        flower_color = random.choice([(255, 255, 255), (255, 200, 50), (255, 150, 180), (200, 200, 255)])
        pygame.draw.circle(surf, flower_color, (x, y), 2)
    return surf
...
def load_game():
    global coins, score, volume, current_lang, display_mode, upgrades, monsters
    try:
        if os.path.exists(SAVE_FILE):
            ...
            # Load display mode (support old saves with fullscreen boolean)
            if "display_mode" in data:
                display_mode = data["display_mode"]
            else:
                fullscreen = data.get("fullscreen", False)
                display_mode = DISPLAY_MODE_FULLSCREEN if fullscreen else DISPLAY_MODE_WINDOWED
...
translations = {
    "en": {
        ...
        "dockyard": "Dockyard",
        "dockyard_desc": "Build a dockyard for boats and fishing!",
        "bought": "Owned",
        "buy": "Buy",
    ...
    "bs": {
        ...
        "dockyard_desc": "Izgradi pristanište na svom ostrvu! - unlocks boats/fishing",
    ...
    "sv": {
        ...
        "languages": "Sprak",
        "dockyard": "Varv",
        "dockyard_desc": "Bygg ett varv pa din o! - unlocks boats/fishing",
    ...
    "pt": {
        ...
        "languages": "Idiomas",
        "dockyard": "Estaleiro",
        "dockyard_desc": "Construa um estaleiro na sua ilha! - unlocks boats/fishing",
    ...
    "ja": {
        ...
        "languages": "Gengo",
        "dockyard": "Dokku",
        "dockyard_desc": "Shima ni dokku o tsukuru! - unlocks boats/fishing",
    ...
    "bg": {
        ...
        "dockyard_desc": "Postroyte pristanishte na ostrova si! - unlocks boats/fishing",
    ...
    "de": {
        ...
        "languages": "Sprachen",
        "dockyard": "Werft",
        "dockyard_desc": "Baue eine Werft auf deiner Insel! - unlocks boats/fishing",
    ...
    "hr": {
        ...
        "dockyard_desc": "Izgradi brodogradilište na svom otoku! - unlocks boats/fishing",
        "check_updates": "Provjeri azuriranja",
        "update_available": "Azuriranje dostupno!",
        "update_version": "Verzija {ver} je spremna!",
        "update_download": "Preuzmi azuriranje",
    ...
    "ro": {
        ...
        "dockyard_desc": "Construieste un santier naval pe insula ta! - unlocks boats/fishing",
    ...
    "es": {
        ...
        "dockyard": "Astillero",
        "dockyard_desc": "¡Construye un astillero en tu isla! - unlocks boats/fishing",
    ...
    "udm": {
        ...
        "dockyard": "Dockyard",
        "dockyard_desc": "Dockyard ostryg shoryos! - unlocks boats/fishing",
...
def render_wrapped_text(surface, text, font, color, x, y, max_width, line_height=None):
    """Render text with word wrapping. Returns total height used."""
    words = text.split(' ')
    lines = []
    current_line = ""
    for word in words:
...
def calculate_wrapped_text_height(text, font, max_width, line_height=None):
    """Calculate height needed for wrapped text without rendering."""
    words = text.split(' ')
    lines = []
    current_line = ""
    for word in words:
...
def draw_island(surface):
    ...
    # Generate grass surface if not already done
    global grass_surface
    if grass_surface is None:
        grass_surface = generate_grass_surface(680, 180)
    surface.blit(grass_surface, (60, 290))
...
def draw_game_ui(surface, coins, score):
    ...
    text_width, text_height = inst.get_width(), inst.get_height()
    padding_x = 20
    box_width = max(200, text_width + padding_x * 2)
    max_width = WIDTH - 40
    box_width = min(box_width, max_width)
    box_height = max(30, text_height + 10)
    box_x = WIDTH // 2 - box_width // 2
    box_y = HEIGHT - 40 - (box_height - 30) // 2  # shift up if taller
    padding_y = (box_height - text_height) // 2
    inst_bg = pygame.Surface((box_width, box_height), pygame.SRCALPHA)
    pygame.draw.rect(inst_bg, (0, 0, 0, 100), (0, 0, box_width, box_height), border_radius=10)
    surface.blit(inst_bg, (box_x, box_y))
    surface.blit(inst, (WIDTH // 2 - text_width // 2, box_y + padding_y))
...
def toggle_fullscreen():
    global display_mode, screen
    # Cycle through modes: windowed -> borderless -> fullscreen -> windowed
...
def apply_display_mode():
    ...
    if display_mode == DISPLAY_MODE_WINDOWED:
        screen = pygame.display.set_mode((WIDTH, HEIGHT))
    elif display_mode == DISPLAY_MODE_BORDERLESS:
        info = pygame.display.Info()
        screen = pygame.display.set_mode((info.current_w, info.current_h), pygame.NOFRAME)
    elif display_mode == DISPLAY_MODE_FULLSCREEN:
        info = pygame.display.Info()
        screen = pygame.display.set_mode((info.current_w, info.current_h), pygame.FULLSCREEN)

def update_fullscreen_button_text():
    if display_mode == DISPLAY_MODE_WINDOWED:
        btn_fullscreen.text = t("display_mode") + ": " + t("windowed")
    elif display_mode == DISPLAY_MODE_BORDERLESS:
        btn_fullscreen.text = t("display_mode") + ": " + t("borderless")
    elif display_mode == DISPLAY_MODE_FULLSCREEN:
        btn_fullscreen.text = t("display_mode") + ": " + t("fullscreen")
...
def draw_shop(surface):
    ...
    # Calculate description heights and box sizes
    desc_max_width = 370  # 400 box width - 15 left padding - 15 right padding
    line_height = font_tiny.get_linesize()
    
    # Get description texts
    hat_desc_text = t("top_hat_desc")
    dock_desc_text = t("dockyard_desc")
    breed_desc_text = t("breeding_structure_desc")
    
    # Calculate required heights
    hat_desc_height = calculate_wrapped_text_height(hat_desc_text, font_tiny, desc_max_width, line_height)
    dock_desc_height = calculate_wrapped_text_height(dock_desc_text, font_tiny, desc_max_width, line_height)
    breed_desc_height = calculate_wrapped_text_height(breed_desc_text, font_tiny, desc_max_width, line_height)
    
    # Calculate box heights (minimum 70, but taller if needed)
    # Name takes 8px, description starts at 32px, need bottom padding of 10px
    hat_box_height = max(70, hat_desc_height + 42)  # 8 + 32 + 2 extra
    dock_box_height = max(70, dock_desc_height + 42)
    breed_box_height = max(70, breed_desc_height + 42)
    
    # Calculate y positions with 15px gaps
    base_y = 170
    hat_y = base_y
    dock_y = hat_y + hat_box_height + 15
    breed_y = dock_y + dock_box_height + 15
    
    # Ensure total height fits in panel (panel ends at y=525, breed_y + breed_box_height should be <= 525)
    total_height = breed_y + breed_box_height - base_y
    if total_height > 355:  # 525 - 170 = 355 available
        # Scale down gaps if needed
        available = 355
        used = hat_box_height + dock_box_height + breed_box_height
        gaps = 2
        if used + gaps * 15 > available:
            # Reduce gaps
            gap = max(5, (available - used) // gaps)
            dock_y = hat_y + hat_box_height + gap
            breed_y = dock_y + dock_box_height + gap
    
    # Shop panel (adjust height based on content)
    panel_height = max(450, breed_y + breed_box_height + 25 - 75)  # 25 bottom padding, 75 panel start y
    panel = pygame.Surface((500, panel_height), pygame.SRCALPHA)
    pygame.draw.rect(panel, (40, 60, 100, 240), (0, 0, 500, panel_height), border_radius=20)
    pygame.draw.rect(panel, ICE_BLUE, (0, 0, 500, panel_height), 3, border_radius=20)
    ...
    # --- Top hat upgrade ---
    pygame.draw.rect(surface, (50, 70, 110), (200, hat_y, 400, hat_box_height), border_radius=10)
    pygame.draw.rect(surface, (80, 100, 150), (200, hat_y, 400, hat_box_height), 2, border_radius=10)

    hat_name = font_small.render(t("top_hat"), True, WHITE)
    surface.blit(hat_name, (215, hat_y + 8))
    render_wrapped_text(surface, hat_desc_text, font_tiny, (180, 200, 220), 215, hat_y + 32, desc_max_width, line_height)

    if upgrades.get("orange_top_hat", False):
        owned_text = font_small.render(t("bought"), True, (100, 255, 100))
        surface.blit(owned_text, (530, hat_y + 22))
    else:
        price_text = font_small.render("50", True, GOLD)
        surface.blit(price_text, (540, hat_y + 22))
        pygame.draw.circle(surface, GOLD, (525, hat_y + 32), 8)

    # --- Dockyard upgrade ---
    pygame.draw.rect(surface, (50, 70, 110), (200, dock_y, 400, dock_box_height), border_radius=10)
    pygame.draw.rect(surface, (80, 100, 150), (200, dock_y, 400, dock_box_height), 2, border_radius=10)

    dock_name = font_small.render(t("dockyard"), True, WHITE)
    surface.blit(dock_name, (215, dock_y + 8))
    render_wrapped_text(surface, dock_desc_text, font_tiny, (180, 200, 220), 215, dock_y + 32, desc_max_width, line_height)

    if upgrades.get("dockyard", False):
        owned_text = font_small.render(t("bought"), True, (100, 255, 100))
        surface.blit(owned_text, (530, dock_y + 22))
    else:
        price_text = font_small.render("100", True, GOLD)
        surface.blit(price_text, (540, dock_y + 22))
        pygame.draw.circle(surface, GOLD, (522, dock_y + 32), 8)

    # --- Breeding Structure upgrade ---
    pygame.draw.rect(surface, (50, 70, 110), (200, breed_y, 400, breed_box_height), border_radius=10)
    pygame.draw.rect(surface, (80, 100, 150), (200, breed_y, 400, breed_box_height), 2, border_radius=10)

    breed_name = font_small.render(t("breeding_structure"), True, WHITE)
    surface.blit(breed_name, (215, breed_y + 8))
    render_wrapped_text(surface, breed_desc_text, font_tiny, (180, 200, 220), 215, breed_y + 32, desc_max_width, line_height)

    if upgrades.get("breeding_structure", False):
        owned_text = font_small.render(t("bought"), True, (100, 255, 100))
        surface.blit(owned_text, (530, breed_y + 22))
    else:
        price_text = font_small.render("200", True, GOLD)
        surface.blit(price_text, (540, breed_y + 22))
        pygame.draw.circle(surface, GOLD, (522, breed_y + 32), 8)
